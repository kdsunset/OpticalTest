...
public class StartExamActivity2 extends AppCompatActivity {
    ...
    private void loadPaper() {
        ...
        new  AsyncTask<Integer, Void, List<Question>>(){


            @Override
            protected List<Question> doInBackground(Integer... integers) {
                PaperDao paperDao = new PaperDao(StartExamActivity2.this);
                List<Question> questionList = paperDao.showALLQuestionOfPaper(integers[0]);

                LogUtils.i("dao");
                LogUtils.i("id="+integers[0]);
            ...
            @Override
            protected void onPostExecute(List<Question> list) {
   ...
   private  class  MyViewPagerListener implements   ViewPager.OnPageChangeListener{
    ...
    private class MyPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            ...
            View convertView = LayoutInflater.from(StartExamActivity2.this).inflate(R.layout.layout_question_item, null);
            ...
            LinearLayout analysis_layout= (LinearLayout) convertView.findViewById(R.id.analysis_layout);
            LinearLayout answerLayout= (LinearLayout) convertView.findViewById(R.id.answer_layout);
            TextView tvAnalysis= (TextView) convertView.findViewById(R.id.tv_analysis);
            TextView tvRightAnswer= (TextView) convertView.findViewById(R.id.tv_right_answer);
            TextView tv_fillinAnswer= (TextView) convertView.findViewById(R.id.tv_fillin_answer);
            if (ACTIVITY_MODE==MODE_CHECK) {
            ...
            int rigans=Integer.valueOf(list.get(position).getAnswer());
            Log.i("", "answer" + ans);
            for (int i = 0; i < 4; i++) {
                Log.i("", "for循环" + i);
                if (ans != -1) {
                   ...
                   if (ACTIVITY_MODE==MODE_CHECK){
                       if (ans==rigans){
                           ...
                           radioButtons[rigans].setTextColor(Color.GREEN);
                           radioButtons[ans].setTextColor(Color.RED);
            ...
            if (ACTIVITY_MODE==MODE_CHECK) {
                ...
                tvRightAnswer.setText("正确答案："+convertAnswer(Integer.valueOf(q.getAnswer())));
                tv_fillinAnswer.setText("你的答案："+convertAnswer(q.getYourAnswer()));
            }
            container.addView(convertView);
            return convertView;

        }
    private  String convertAnswer(int ans){
        String ansString="";
        switch (ans){
            case 0:
                ansString="A";
